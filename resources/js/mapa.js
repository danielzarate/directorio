...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(document.querySelector('#mapa')) {
        ...
        const lat = document.querySelector('#lat').value === '' ? 4.5969488 : document.querySelector('#lat').value;
        const lng = document.querySelector('#lng').value === '' ? -74.0737128 : document.querySelector('#lng').value;
       ...
       buscador.addEventListener('blur', buscarDireccion);
        ...
        function buscarDireccion(e) {
            ...
            if(e.target.value.length > 1) {
                provider.search({query: e.target.value + ' Bogota COL ' })
                    .then( resultado => {
                        if( resultado  ){
